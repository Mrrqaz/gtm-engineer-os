...
Log the score and layer breakdown per account. Only accounts at 60+ move to Step 2.

## Step 2: Score Live Signals (Time-Windowed)

For each ICP-gated account, check for each signal type below. If the data source for a signal type wasn't provided this run, mark it "not checked" — don't assume absence means the signal didn't happen.
...
- **30+ points, or any 2-signal combo that includes a job change or funding event** → **Immediate outreach.** These get flagged at the top of the queue and the outreach angle should reference the specific signal, not a generic opener.

The named-combo override exists because a job-change-plus-funding stack (new decision-maker with new budget) converts disproportionately better than the raw point sum suggests — ColdIQ's source material treats this combo as its own tier, and that holds here.
...
Accounts checked: 3
Gated out at ICP: 1 (Northfield Ops — Layer 1 (Firmographic) 18/40, under
  50 employees, below segment floor)
...
- The named job-change + funding combo overrides the raw point sum into Immediate Outreach, per Step 3. Don't apply that override to other combos without the same evidence base.
