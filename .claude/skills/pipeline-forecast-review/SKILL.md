...
2. **Deal-desk sync.** Run Steps 1–4 below. Flag anomalies — a deal marked Commit with no signed paperwork, a stage that doesn't match the tier it's claiming, a close date that's slipped without a stage change. This sync is where mismatches get resolved before numbers go up the chain.
...
```
Pipeline Forecast Review — week of 2026-06-30
...
1. Ferro Systems (Commit, $42k) — stalled 16 days, single-threaded on the
   ...
   largest deal in Commit and it's showing two risk signals at once — needs
