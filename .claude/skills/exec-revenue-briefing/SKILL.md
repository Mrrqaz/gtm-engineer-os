...
| **Pipeline coverage ratio** | Is there enough open pipeline in the current period to plausibly hit the number? (open pipeline ÷ remaining target) | Watch: below 4x (the same new-logo guardrail `pipeline-forecast-review` enforces upstream). Red: below 2x with less than 6 weeks left in the period. |
...
## Step 6: Assemble the Output and Choose Mode
...
```
Rivergate Data Revenue Briefing - Weekly - Week of 2026-06-29
...
Headline: Two late-stage deals totaling $180K are carried at 80% probability with
