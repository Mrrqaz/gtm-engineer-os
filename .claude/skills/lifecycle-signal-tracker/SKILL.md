...
**Adapted from:** t0ddc3by's `claude-for-customer-success` (github.com/t0ddc3by/claude-for-customer-success, 81 skills): the churn root-cause taxonomy and corroboration-driven confidence tiers are adapted from its `churn-rca` skill, and the time-to-value pace multiplier (including the >1.15 at-risk threshold) from its `ttv-analysis` skill. Merging the two into one lifecycle pass, the expansion-ready flag, and the Rivergate Data adaptation are this version's changes.
...
**Confidence tiers** are set by how many independent sources corroborate the classification, not by how compelling any single source sounds:
