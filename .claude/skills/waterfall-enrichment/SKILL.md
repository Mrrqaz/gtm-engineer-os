...
- **Phase 1 is binary, not a score.** MATCH or no-match against explicit fit criteria (industry, headcount band, tech stack signal, funding stage, geography). Don't produce a numeric ICP score here. A score invites borderline accounts to sneak through on a "70/100 is close enough" rationalization at exactly the step where money starts being spent. This is a different job from `signal-prospecting`'s 100-point ICP gate, which ranks discovery priority across many accounts before any of them are worth spending on; by the time an account reaches this skill the only question left is a binary go/no-go on spend. If a criterion is ambiguous, that's a MATCH with a caveat noted, not a fractional score.
...
- **Every spent credit gets logged against the lead it was spent on.** No exceptions, including a provider miss (a miss still cost a credit or an API call in most pricing models, so log it as a miss, not silently).
